...
- **Deployed app:** https://live-meeting-assistant.vercel.app/
- **GitHub repository:** https://github.com/Preetham134/live_meeting_assistant
...
### 5. Editable settings
The app includes a top settings panel where users can modify prompts and runtime parameters without changing code.

Editable settings include:
- Live Suggestion Prompt: Prompt used to generate real-time meeting suggestions from the transcript context.
- Chat Prompt: Prompt used to generate detailed chat responses when a user clicks a suggestion or asks a question manually.
- Suggestion Window: Number of recent transcript chunks used to generate each live suggestion batch.
- Clicked After Chunks: Number of transcript chunks after the original suggestion snapshot used when explaining a clicked suggestion. This helps preserve context if the user clicks an older suggestion later.
- Manual Chat Window: Number of latest transcript chunks used when answering manually typed chat questions, along with the running meeting summary.
- Transcription Chunk Duration: Time interval, in seconds, used to split microphone audio into transcription chunks.
- Suggestion Refresh Interval: Time interval, in seconds, used to automatically generate a fresh batch of live suggestions.
- temperature
- suggestion model
- chat model
- transcription model
...
I chose React because the project emphasizes a live product-like experience with three columns: transcript, suggestions, and chat. React made it easier to manage microphone state, timers, transcript updates, suggestion batches, chat history, and settings from a single client-side app.

I intentionally avoided a backend to keep the implementation lightweight and focused on the project goals: prompt quality, context strategy, responsiveness, and clean UI. The Groq API key is entered by the user in the UI and is never hardcoded in the repo.
...
```bash
git clone https://github.com/Preetham134/live_meeting_assistant
```
...
True audio streaming would require more infrastructure and more edge-case handling. For this project, I implemented chunk-based live transcription using `MediaRecorder`. It provides a live experience with acceptable delay and simpler, more reliable browser behavior.

### 2. Client-only architecture

The app calls Groq directly from the browser using the user-provided API key. This keeps setup simple and avoids managing a backend. In production, I would move API calls to a backend to avoid exposing keys in browser requests and to add rate limiting, logging, and stronger error handling.
...
This is intended to make the user can inspect what was said, what suggestions were shown, what was clicked, and how the chat answered.
